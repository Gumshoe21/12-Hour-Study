...
export const login =
  ...
  async (dispatch) => {
    ...
    try {
      const req = await axios.post(`${process.env.REACT_APP_API_URL}/api/${APIVERSION}/users/login`, body, config)
...
export const logout = () => async (dispatch) => {
  ...
  try {
    const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/${APIVERSION}/users/logout`, config)
    dispatch(authSlice.actions.logout())
  } catch (err) {}
}

export const register =
  ({ username, email, password, passwordConfirm, timezone }) =>
  async (dispatch) => {
    const config = {
      headers: {
        'Content-Type': 'application/json',
    ...
    try {
      const req = await axios.post(`${process.env.REACT_APP_API_URL}/api/${APIVERSION}/users/signup`, body, config)
      /*
...
export const updateUser =
  ...
  async (dispatch) => {
    ...
    try {
      const res = await axios.patch(`${process.env.REACT_APP_API_URL}/api/${APIVERSION}/users/${auth.user._id}`, body, config)
...
export const forgotPassword =
  ...
  async (dispatch) => {
    ...
    try {
      const req = await axios.post(`${process.env.REACT_APP_API_URL}/api/${APIVERSION}/users/forgotPassword`, body, config)
...
export const resetPassword =
  ...
  async (dispatch) => {
    ...
    try {
      const req = await axios.patch(`/api/${APIVERSION}/users/resetPassword/${token}`, body, config)
      const email = await req.data.user.email
      body = await JSON.stringify({ password, email })
      const login = await axios.post(`/api/${APIVERSION}/users/login`, body, config)
